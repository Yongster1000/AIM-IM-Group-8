...
class Classifier(nn.Module):
    def __init__(self, d_model=144, n_class=2, dropout=0.1, cls = None, num_range = 0):
        ...
        #self.embedding_for_num = nn.Embedding(12 + 1, d_model)
        self.embedding_for_num = nn.Linear(1, d_model)
        self.linear_modules = nn.ModuleList([nn.Linear(1, d_model) for _ in range(12 + 1)])
        '''
        self.embedding_for_num = nn.Sequential(
    ...
    def forward(self, mels, mask = None, transformer_explainer = None):
        ...
        m_vector = self.linear_modules[0](self.linear(mels[:, 20:40].float()).unsqueeze(1))# * self.embedding_for_num(torch.tensor([12]).to(device)).unsqueeze(0)
        cat_embed_vectors = self.embedding_for_cat(mels[:, :8].int()) #(B, (N+1)*S, D)
        num_embed_vectors = torch.cat([self.linear_modules[i](mels[:, i + 7:i + 7 + 1].float().unsqueeze(1)) for i in range(1, 12 + 1)], dim = 1)
        #num_embed_vectors = self.embedding_for_num(torch.arange(12).to(device)).unsqueeze(0) * mels[:, 8:20].unsqueeze(-1)
        ...
        #print(cat_embed_vectors.shape, num_embed_vectors.shape, m_vector.shape)
        out = torch.cat((cat_embed_vectors, num_embed_vectors, m_vector), dim = 1)
...
def main(
    ...
    if not save:
        ...
        if test:
            ...
            #test_set = torch.load("test_data_m.pt") #Note we conduct the explainability evaluation method on valid dataset
            #test_set = torch.from_numpy(df_test.to_numpy())
...
def parse_args():
  ...
  config = {
    ...
    "summary_type": "mean",
    "save": True,
    "save_path": "test_with_methylation.ckpt",#0716, 6 layer#0805, 43
    "batch_size": 256,
    "n_workers": 0,
